...
def find_candidates_from_ngrams(ngram_words, master_set, max_n, thresh, extra_seed=None,disable_single_word_fuzzy=False ):
    """
    Generic helper:
    ...
    for n in range(1, max_n + 1):
        ...
        for g in grams_n:
            ...
            if disable_single_word_fuzzy and n == 1:
                continue
...
def main(limit=1000, excel=None, batch_size=200):
    ...
    with eng.begin() as con:
        ...
        inputs = pd.read_sql("SELECT * FROM input.addresses ORDER BY id LIMIT {}".format(int(limit)), con)
    ...
    city_master = set(postal["city"]).union(rta["city"]).union(world["city"])
    country_master = set(postal["country"]).union(world["country"]).union({"India"})
    ...
    for start in range(0, len(inputs), batch_size):
        ...
        for _, rec in chunk.iterrows():
            ...
            # 1) Whole address
            whole = norm_text(" ".join([str(x or "") for x in [a1, a2, a3, in_city, in_state_expanded, in_country, in_pin]]))
            ...
            city_cands = find_candidates_from_ngrams(
                city_words, city_master, max_n=2, thresh=CITY_STATE_THRESH, extra_seed=in_city,disable_single_word_fuzzy=True
            )
            state_cands_raw = find_candidates_from_ngrams(
                state_words, set(state_alias.keys()), max_n=2, thresh=CITY_STATE_THRESH, extra_seed=in_state_expanded,disable_single_word_fuzzy=False
            )
            state_cands = set()
            ...
            if pins_text:
                    ...
                    if len(pins_text) == 1:
                        ...
                        for p in pins_text:
                            ...
                            if not rows:
                                ...
                                continue
                            for src, row in rows:
                                s_city = max((sim(row["city"], c) for c in city_cands), default=0.0)
                                s_state = max((sim(row["state"], s) for s in state_cands), default=0.0)
                                s_ctry = max((sim(row["country"], k) for k in country_cands), default=0.0)
                ...
                probably_india = (
                    Title(in_country) == "India"
                    or "India" in {Title(k) for k in country_cands}
                )
                if probably_india and city_cands:
                    for src, df_src in [("postal", postal), ("rta", rta)]:
                        sub = df_src[df_src["city"].isin(city_cands)]
                        for _, row in sub.iterrows():
                            pins_from_city.add(str(row["pincode"]))
                            audits.append(
                                {
                                    "input_id": rid,
                                    "type": "city→pin",
                ...
                if pins_from_city:
                    ...
                    for p in pins_from_city:
                        for src, row in pin_index.get(p, []):
                            s_state = max((sim(row["state"], s) for s in state_cands), default=0.0)
                            s_ctry  = max((sim(row["country"], k) for k in country_cands), default=0.0)
