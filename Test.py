...
FALLBACK_STATE_ABBR = {
    ...
    "DD":"Daman And Diu","DL":"Delhi","DN":"Dadra And Nagar Haveli","GA":"Goa","GJ":"Gujarat","HP":"Himachal Pradesh",
    "HR":"Haryana","JH":"Jharkhand","JK":"Jammu And Kashmir","KA":"Karnataka","KL":"Kerala","LA":"Ladakh",
    "LD":"Lakshadweep","MH":"Maharashtra","ML":"Meghalaya","MN":"Manipur","MP":"Madhya Pradesh","MZ":"Mizoram",
    "NL":"Nagaland","OD":"Odisha","OR":"Odisha","PB":"Punjab","PY":"Puducherry","RJ":"Rajasthan","SK":"Sikkim",
    "TG":"Telangana","TS":"Telangana","TN":"Tamil Nadu","TR":"Tripura","UK":"Uttarakhand","UA":"Uttarakhand",
    "UP":"Uttar Pradesh","WB":"West Bengal","UT":"Uttarakhand"
}
...
def expand_state_abbrev(state_in, states_df):
    ...
    # 1) Exact abbreviation match (DB first)
    hits = states_df[states_df["abbreviation"] == tok]
    if not hits.empty: return Title(hits.iloc[0]["state"])

    # 1b) Fallback map if DB lacks it
    if tok in FALLBACK_STATE_ABBR:
        return Title(FALLBACK_STATE_ABBR[tok])

    # 2) Exact full name match
    hits = states_df[states_df["state"] == s_title]
    if not hits.empty: return Title(hits.iloc[0]["state"])

    # 3) Tokenized name match (TamilNadu)
...
def main(limit=1000, excel=None):
    ...
    for _, r in inputs.iterrows():
        ...
        # PRIORITY: if input has a 6-digit PIN present in DB, take it directly
        if in_pin and in_pin in pin_index:
            rows = pin_index.get(in_pin) or []
            # choose the first row; later scoring will still set binary pins correctly
            if rows:
                src0, rr0 = rows[0]
                out_pin = in_pin
                out_city = Title(rr0['city'])
                out_state = Title(rr0['state'])
                out_country = Title(rr0['country'])
                source_used = 'pincode(exact)'
                # set 6 binary flags w.r.t input comparisons
                from_sim_city  = sim(out_city, Title(r.get('city'))) if r.get('city') else 0
                from_sim_state = sim(out_state, expand_state_abbrev(Title(r.get('state')), states)) if r.get('state') else 0
                from_sim_ctry  = sim(out_country, Title(r.get('country'))) if r.get('country') else 0
                P_input_city   = 1 if from_sim_city  >= THRESH else 0
                P_city_db      = 1
                P_input_state  = 1 if from_sim_state >= THRESH else 0
                P_state_db     = 1
                P_input_country= 1 if from_sim_ctry  >= THRESH else 0
                P_country_db   = 1
