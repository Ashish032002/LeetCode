...
def main(limit=1000, excel=None, batch_size=200):
    ...
    # --- IMPORTANT: split India masters vs world masters ---
    city_master_india = set(postal["city"]).union(set(rta["city"]))
    country_master_india = set(postal["country"]).union({"India"})

    world_city_master = set(world["city"])
    world_country_master = set(world["country"])

    # Global (for foreign addresses)
    city_master_global = city_master_india.union(world_city_master)
    country_master_global = country_master_india.union(world_country_master)

    # Build pin index for quick look-up (using both postal + rta)
    pin_index = {}
    ...
    for start in range(0, len(inputs), batch_size):
        ...
        for _, rec in chunk.iterrows():
            ...
            # 2) Decide whether to use world cities or only India
            # if input country or any country word indicates India -> ONLY India masters
            lower_country_words = [w.lower() for w in words]
            has_india_hint = ("india" in lower_country_words) or (in_country.lower() == "india")

            if has_india_hint:
                row_city_master = city_master_india
                row_country_master = country_master_india
            else:
                row_city_master = city_master_global
                row_country_master = country_master_global

            # 3) N-grams: city/state 1-2; country 1-4
            ...
            # 4) Pincodes from text – ABSOLUTE priority, no fuzzy
            ...
            # 5) Choose city/state/country using chosen_pin_row if available
            ...
            # 7) local_address = whole – entities we already found
            remove_list = []
            ...
            # 8) Scores: 3 rules x 3 entities + overall
