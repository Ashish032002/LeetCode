...
def main(limit=1000, excel=None, batch_size=200):
    ...
    for start in range(0, len(inputs), batch_size):
        ...
        for _, rec in chunk.iterrows():
            ...
            # --- INDIA-FIRST LOGIC ---
            # Pre-scan for 'India' to decide which master lists to use.
            temp_country_cands = find_candidates_from_ngrams(
                country_words,
                country_master_full, # Use full list just for this check
                max_n=4,
            ...
            # If it's likely an Indian address, use the restricted India-only master lists.
            # This is the core change to prevent fuzzy matching on foreign entities.
            city_master = city_master_india if probably_india else city_master_world
            country_master = country_master_india_only if probably_india else country_master_full
            ...
            # Track whether user-provided pincode is invalid or not found
            invalid_user_pin = False

            if pins_text:
                # Collect only 6-digit tokens from the text for actual pin candidates
                valid_text_pins = {p for p in pins_text if re.fullmatch(r"\d{6}", p)}

                # 3.1 Handle the input pincode field first (if provided)
                if in_pin and not is_null_token(in_pin):
                    if not re.fullmatch(r"\d{6}", in_pin):
                        # Wrong format (like 5-digit) -> do NOT pick any pin, just mark reason
                        invalid_user_pin = True
                        reasons.append("invalid_pincode_format")
                    else:
                        # Correct 6-digit format – check against master
                        ...
                        if rows:
                            ...
                            # Format is valid but pincode not present in master tables
                            invalid_user_pin = True
                            reasons.append("pincode_not_found_in_master")

                # 3.2 If we don't have a chosen_pin yet AND user pin is not invalid,
                #     we are allowed to choose from valid pins found in text.
                if (chosen_pin is None) and (not invalid_user_pin) and valid_text_pins:
                    if len(valid_text_pins) == 1:
                        only_pin = next(iter(valid_text_pins))
                        ...
                        # Multiple valid pins – choose the one that best matches city/state/country via DB.
                        best = None
                        best_score = -1.0
                        for p in valid_text_pins:
                ...
                # 3.3 Build all_possible_pincodes_set
                if invalid_user_pin:
                    # When user gave an invalid/unknown pincode, do NOT choose any pin.
                    # Only derive all possible pincodes from city/state via DB.
                    chosen_pin = None
                    chosen_pin_row = None
                    pins_from_city = set()
                    if city_cands:
                    ...
                    # Normal case: we can still record all valid pins we saw in text.
                    if valid_text_pins:
                        all_possible_pincodes_set = set(valid_text_pins)
                    else:
                        # Fallback: no 6-digit pins in text – derive from city/state.
                        pins_from_city = set()
                        if city_cands:
                            for src, df_src in [("postal", postal), ("rta", rta)]:
                                sub = df_src[df_src["city"].isin(city_cands)]
                ...
                # No pin in text at all – derive from discovered cities via DB.
                pins_from_city = set()
                if city_cands:
                    for src, df_src in [("postal", postal), ("rta", rta)]:
                        sub = df_src[df_src["city"].isin(city_cands)]
...
# 4) Choose city/state/country – PINCODE ROW HAS HIGHEST PRIORIT
            ...
            # -------- Strong India Lock Based on STATE (FINAL CORRECT POSITION) --------
            ...
            # Enrich countries from cities/states ONLY if it's not a confirmed Indian address
            if chosen_country == "India":
                foreign_country_possible = 0
                is_confirmed_india = True

                # Countries: Only India
                all_possible_countries = ["India"]

                # Cities: Remove any world_cities (only keep Indian ones)
                indian_city_set = set(city_master_india)
                all_possible_cities = [
                    c for c in all_possible_cities
                    if c in indian_city_set
                ...
                for ci in list(all_possible_cities):
                    for co in city_to_countries.get(ci, []):
                        if co not in all_possible_countries:
                            all_possible_countries.append(co)
                        elif "India" not in all_possible_countries:
                            all_possible_countries = ["India"]
...
def validate_inputs_df(inputs, batch_size=200):
    eng = get_db_connection()
    ...
    # 'inputs' is expected to be a pandas DataFrame with address columns already loaded.
    # Normalise reference text
    for df, cols in [
        (postal, ["city", "state", "country"]),
        (rta, ["city", "state", "country"]),
